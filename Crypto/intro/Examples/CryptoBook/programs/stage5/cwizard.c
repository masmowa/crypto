...
typedef enum {ALL_CHARS, PUNCT_NOT_WHITE, WHITE_NOT_PUNCT, 
	      NOT_WHITE_NOT_PUNCT, NO_TYPE} t_count;
...
void print_usage() {
  fprintf(stderr, "Usage: cwizard [-h] [-bf <filename>] [-pf <filename>]\n");
...
t_status read_text(FILE *infile,  
  ...
  while ((c = fgetc(infile)) != EOF) { 
	...
	fprintf(stderr, "Warning: Unrecognized character %c (%d).\n", 
		c, (int) c);
      }
...
t_status search(FILE *infile, 
  ...
  wc = w_current;
  for (type = ALL_CHARS; type < NO_TYPE; type++) {

    w_current = wc;
    t_index = (words[w_current - 1] + 1) % t_size; 
	...
	words[w_current] = t_index;
	w_current++;

      } else if (type == PUNCT_NOT_WHITE) {
	...
	if (text[t_index] != '#') {
      ...
      } else if (type == NOT_WHITE_NOT_PUNCT) {

	if ((text[t_index] != ' ') && (text[t_index] != '#')) {
    ...
    evaluate_plain_text(outfile, words, text, t_size, t_first, t_last);
  }
